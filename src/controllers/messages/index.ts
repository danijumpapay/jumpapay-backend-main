...
export const listMessages = async (req: Request, res: Response) => {
  ...
  try {
    const query: any = {
      bool: {
        must: [{ match: from ? { from } : { room_id: roomId } }],
      },
    };
    ...
    const executeAll = await Promise.all([
      whatsappSchema.Messages.query()
        ...
        .page(page - 1, limit),
      whatsappSchema.Chats.query().select("id", "password", "name", "avatar").findOne("id", roomId),
    ]);
    ...
    if (!isError) {
      if (page === 1) {
        const updateChat = whatsappSchema.Chats.query()
          .patch({
        ...
        const updateBulkMessage = whatsappSchema.Messages.query()
          .patch({
            is_read: true,
          })
          .where("room_id", roomId);

        await Promise.all([updateChat, updateBulkMessage]);
      }
    ...
    return res.status(500).json(errorResponseOld(error.message || "Internal Server Error", {}));
  }
};
//#endregion - listData
...
export const sendMessage = async (req: Request, res: Response) => {
  ...
  const documents = files.filter(
    (f) => f.mimetype.startsWith("application/") || f.mimetype === "text/plain"
  );

  if (audios.length > 1) {
    return res.status(400).json(errorResponseOld("Only one audio file allowed", {}));
  }
    ...
    if (!isError) {
      if (files.length === 0) {
        ...
        if (images.length > 0) {
          ...
          return res.status(401).json(errorResponseOld("Your file is not supported", {}));
        }
      }
    } else {
      return res
...
export const takeChat = async (req: Request, res: Response) => {
  ...
  try {
    const updateChat = await whatsappSchema.Chats.query()
      .patch({
    ...
    if (!updateChat) {
      return res.status(404).json(errorResponseOld("Chat not found", {}));
    }
