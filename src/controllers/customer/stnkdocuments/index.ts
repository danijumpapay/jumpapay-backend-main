    ...
    const rawQuery = customer.StnkDocuments.querySoftDelete()
      ...
      .modify((queryBuilder) => {
        if (searchKeywords !== null && searchKeywords !== "") {
          queryBuilder.whereRaw("LOWER(customer.stnk_documents.stnk_number) LIKE ?", [
            `%${searchKeywords}%`,
          ]);
        }
      });
...
export const detailData = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (stnk) {
      res.status(200).json(successResponseOld("SUCCESS", { results: stnk }));
    } else {
...
export const createData = async (req: Request, res: Response) => {
  const { userId, vehicleId, stnkNumber, issueDate, expiryDate, taxDueDate, isActive, image } =
    req.body;
  ...
  try {
    ...
    res.status(201).json(successResponseOld("SUCCESS", { errors: null, results: stnk }));
  } catch (error: unknown) {
    if (error instanceof Error) {
...
export const updateData = async (req: Request, res: Response) => {
  ...
  try {
    const updated = await customer.StnkDocuments.querySoftDelete().findById(stnkId).patch({
      user_id: userId,
      vehicle_id: vehicleId,
      stnk_number: stnkNumber,
      issue_date: issueDate,
